...
import {ParamsTsCode, Daily, DailyBasic} from './apiTypes';
...
const marketDailyOfWeek = tool<ParamsTsCode, Daily[]>({
...
const marketDailyOfMonth = tool<ParamsTsCode, Daily[]>({
...
const fundamentalDailyOfWeek = tool<ParamsTsCode, DailyBasic[]>({
...
const fundamentalDailyOfMonth = tool<ParamsTsCode, DailyBasic[]>({
    description: '获取股票的基本面数据，输入股票代码，按天返回该股票过去一周的基本面数据',
    inputSchema: z.object({ts_code}),
    outputSchema: dailyBasicSchema,
